"""
Explore the relationship between crash LRSNUMBER and AADT StandardRouteCode.

Strategy: use the AOTROUTE field in crash data as a bridge.
Both crash data and AADT data reference Vermont routes by name/number.
"""
...
# e.g., LRSNUMBER=20 might correspond to StandardRouteCode containing "020" or "US-2"
print("\n  Checking numeric extraction from StandardRouteCode...")
aadt_nums = {}
for code in sorted(aadt["StandardRouteCode"].dropna().unique()):
    # Extract route number patterns
    parts = code.split("-")
    if len(parts) >= 2:
...
# Group crashes by LRSNUMBER, get milepoint ranges
